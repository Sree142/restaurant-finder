...
struct RestDist {
  int16_t index; // Index of restaurant from 0 to NUM_RESTAURANTS-1.
  int16_t dist;  // Manhatten distance to cursor position.
};
...
// Assumes *card has been initialized for raw reads.
int getAndSortRestaurants(const MapView& mv, RestDist restaurants[], int* i, int* mode,
